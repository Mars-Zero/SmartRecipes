# SmartRecipes
Pentru rulare: In /client se va rula comanda "npm run build" pentru a se crea un build actual al frontend-ului. Apoi, se va rula "python3 run.py" pentru a porni backend-ul.

Am implementat aproape toate paginile, singurele pe care nu am reusit sa le implementez calumea sunt cea de profil si paginile pentru fiecare reteta in parte. Am implemntat conform design-ului. Pe backend am implementat aproape tot, mai putin rating-urile pentru retete si pozele pentru retete. Am login functional cu JWT, baza de date sql.
